...
// RouterInterfaceInformer provides access to a shared informer and lister for
// RouterInterfaces.
...
// NewRouterInterfaceInformer constructs a new informer for RouterInterface type.
...
func NewRouterInterfaceInformer(client versioned.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredRouterInterfaceInformer(client, namespace, resyncPeriod, indexers, nil)
}

// NewFilteredRouterInterfaceInformer constructs a new informer for RouterInterface type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.ComputeV1alpha1().RouterInterfaces(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.ComputeV1alpha1().RouterInterfaces(namespace).Watch(context.TODO(), options)
			},
		},
		&computev1alpha1.RouterInterface{},
		resyncPeriod,
		indexers,
	)
}

func (f *routerInterfaceInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredRouterInterfaceInformer(client, f.namespace, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, f.tweakListOptions)
}

func (f *routerInterfaceInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&computev1alpha1.RouterInterface{}, f.defaultInformer)
}
