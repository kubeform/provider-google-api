/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
	v1alpha1 "kubeform.dev/provider-google-api/apis/spanner/v1alpha1"
)

// FakeDatabaseIamMembers implements DatabaseIamMemberInterface
type FakeDatabaseIamMembers struct {
	Fake *FakeSpannerV1alpha1
	ns   string
}

var databaseiammembersResource = schema.GroupVersionResource{Group: "spanner.google.kubeform.com", Version: "v1alpha1", Resource: "databaseiammembers"}

var databaseiammembersKind = schema.GroupVersionKind{Group: "spanner.google.kubeform.com", Version: "v1alpha1", Kind: "DatabaseIamMember"}

// Get takes name of the databaseIamMember, and returns the corresponding databaseIamMember object, and an error if there is any.
func (c *FakeDatabaseIamMembers) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.DatabaseIamMember, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(databaseiammembersResource, c.ns, name), &v1alpha1.DatabaseIamMember{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DatabaseIamMember), err
}

// List takes label and field selectors, and returns the list of DatabaseIamMembers that match those selectors.
func (c *FakeDatabaseIamMembers) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.DatabaseIamMemberList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(databaseiammembersResource, databaseiammembersKind, c.ns, opts), &v1alpha1.DatabaseIamMemberList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.DatabaseIamMemberList{ListMeta: obj.(*v1alpha1.DatabaseIamMemberList).ListMeta}
	for _, item := range obj.(*v1alpha1.DatabaseIamMemberList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested databaseIamMembers.
func (c *FakeDatabaseIamMembers) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(databaseiammembersResource, c.ns, opts))

}

// Create takes the representation of a databaseIamMember and creates it.  Returns the server's representation of the databaseIamMember, and an error, if there is any.
func (c *FakeDatabaseIamMembers) Create(ctx context.Context, databaseIamMember *v1alpha1.DatabaseIamMember, opts v1.CreateOptions) (result *v1alpha1.DatabaseIamMember, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(databaseiammembersResource, c.ns, databaseIamMember), &v1alpha1.DatabaseIamMember{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DatabaseIamMember), err
}

// Update takes the representation of a databaseIamMember and updates it. Returns the server's representation of the databaseIamMember, and an error, if there is any.
func (c *FakeDatabaseIamMembers) Update(ctx context.Context, databaseIamMember *v1alpha1.DatabaseIamMember, opts v1.UpdateOptions) (result *v1alpha1.DatabaseIamMember, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(databaseiammembersResource, c.ns, databaseIamMember), &v1alpha1.DatabaseIamMember{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DatabaseIamMember), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeDatabaseIamMembers) UpdateStatus(ctx context.Context, databaseIamMember *v1alpha1.DatabaseIamMember, opts v1.UpdateOptions) (*v1alpha1.DatabaseIamMember, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(databaseiammembersResource, "status", c.ns, databaseIamMember), &v1alpha1.DatabaseIamMember{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DatabaseIamMember), err
}

// Delete takes name of the databaseIamMember and deletes it. Returns an error if one occurs.
func (c *FakeDatabaseIamMembers) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(databaseiammembersResource, c.ns, name), &v1alpha1.DatabaseIamMember{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeDatabaseIamMembers) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(databaseiammembersResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.DatabaseIamMemberList{})
	return err
}

// Patch applies the patch and returns the patched databaseIamMember.
func (c *FakeDatabaseIamMembers) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.DatabaseIamMember, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(databaseiammembersResource, c.ns, name, pt, data, subresources...), &v1alpha1.DatabaseIamMember{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DatabaseIamMember), err
}
