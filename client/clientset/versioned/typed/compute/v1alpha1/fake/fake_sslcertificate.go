/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "kubeform.dev/provider-google-api/apis/compute/v1alpha1"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeSslCertificates implements SslCertificateInterface
type FakeSslCertificates struct {
	Fake *FakeComputeV1alpha1
	ns   string
}

var sslcertificatesResource = schema.GroupVersionResource{Group: "compute.google.kubeform.com", Version: "v1alpha1", Resource: "sslcertificates"}

var sslcertificatesKind = schema.GroupVersionKind{Group: "compute.google.kubeform.com", Version: "v1alpha1", Kind: "SslCertificate"}

// Get takes name of the sslCertificate, and returns the corresponding sslCertificate object, and an error if there is any.
func (c *FakeSslCertificates) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.SslCertificate, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(sslcertificatesResource, c.ns, name), &v1alpha1.SslCertificate{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.SslCertificate), err
}

// List takes label and field selectors, and returns the list of SslCertificates that match those selectors.
func (c *FakeSslCertificates) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.SslCertificateList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(sslcertificatesResource, sslcertificatesKind, c.ns, opts), &v1alpha1.SslCertificateList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.SslCertificateList{ListMeta: obj.(*v1alpha1.SslCertificateList).ListMeta}
	for _, item := range obj.(*v1alpha1.SslCertificateList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested sslCertificates.
func (c *FakeSslCertificates) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(sslcertificatesResource, c.ns, opts))

}

// Create takes the representation of a sslCertificate and creates it.  Returns the server's representation of the sslCertificate, and an error, if there is any.
func (c *FakeSslCertificates) Create(ctx context.Context, sslCertificate *v1alpha1.SslCertificate, opts v1.CreateOptions) (result *v1alpha1.SslCertificate, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(sslcertificatesResource, c.ns, sslCertificate), &v1alpha1.SslCertificate{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.SslCertificate), err
}

// Update takes the representation of a sslCertificate and updates it. Returns the server's representation of the sslCertificate, and an error, if there is any.
func (c *FakeSslCertificates) Update(ctx context.Context, sslCertificate *v1alpha1.SslCertificate, opts v1.UpdateOptions) (result *v1alpha1.SslCertificate, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(sslcertificatesResource, c.ns, sslCertificate), &v1alpha1.SslCertificate{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.SslCertificate), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeSslCertificates) UpdateStatus(ctx context.Context, sslCertificate *v1alpha1.SslCertificate, opts v1.UpdateOptions) (*v1alpha1.SslCertificate, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(sslcertificatesResource, "status", c.ns, sslCertificate), &v1alpha1.SslCertificate{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.SslCertificate), err
}

// Delete takes name of the sslCertificate and deletes it. Returns an error if one occurs.
func (c *FakeSslCertificates) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(sslcertificatesResource, c.ns, name), &v1alpha1.SslCertificate{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeSslCertificates) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(sslcertificatesResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.SslCertificateList{})
	return err
}

// Patch applies the patch and returns the patched sslCertificate.
func (c *FakeSslCertificates) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.SslCertificate, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(sslcertificatesResource, c.ns, name, pt, data, subresources...), &v1alpha1.SslCertificate{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.SslCertificate), err
}
