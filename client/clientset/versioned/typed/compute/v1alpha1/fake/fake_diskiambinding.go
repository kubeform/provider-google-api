/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "kubeform.dev/provider-google-api/apis/compute/v1alpha1"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeDiskIamBindings implements DiskIamBindingInterface
type FakeDiskIamBindings struct {
	Fake *FakeComputeV1alpha1
	ns   string
}

var diskiambindingsResource = schema.GroupVersionResource{Group: "compute.google.kubeform.com", Version: "v1alpha1", Resource: "diskiambindings"}

var diskiambindingsKind = schema.GroupVersionKind{Group: "compute.google.kubeform.com", Version: "v1alpha1", Kind: "DiskIamBinding"}

// Get takes name of the diskIamBinding, and returns the corresponding diskIamBinding object, and an error if there is any.
func (c *FakeDiskIamBindings) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.DiskIamBinding, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(diskiambindingsResource, c.ns, name), &v1alpha1.DiskIamBinding{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DiskIamBinding), err
}

// List takes label and field selectors, and returns the list of DiskIamBindings that match those selectors.
func (c *FakeDiskIamBindings) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.DiskIamBindingList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(diskiambindingsResource, diskiambindingsKind, c.ns, opts), &v1alpha1.DiskIamBindingList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.DiskIamBindingList{ListMeta: obj.(*v1alpha1.DiskIamBindingList).ListMeta}
	for _, item := range obj.(*v1alpha1.DiskIamBindingList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested diskIamBindings.
func (c *FakeDiskIamBindings) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(diskiambindingsResource, c.ns, opts))

}

// Create takes the representation of a diskIamBinding and creates it.  Returns the server's representation of the diskIamBinding, and an error, if there is any.
func (c *FakeDiskIamBindings) Create(ctx context.Context, diskIamBinding *v1alpha1.DiskIamBinding, opts v1.CreateOptions) (result *v1alpha1.DiskIamBinding, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(diskiambindingsResource, c.ns, diskIamBinding), &v1alpha1.DiskIamBinding{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DiskIamBinding), err
}

// Update takes the representation of a diskIamBinding and updates it. Returns the server's representation of the diskIamBinding, and an error, if there is any.
func (c *FakeDiskIamBindings) Update(ctx context.Context, diskIamBinding *v1alpha1.DiskIamBinding, opts v1.UpdateOptions) (result *v1alpha1.DiskIamBinding, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(diskiambindingsResource, c.ns, diskIamBinding), &v1alpha1.DiskIamBinding{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DiskIamBinding), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeDiskIamBindings) UpdateStatus(ctx context.Context, diskIamBinding *v1alpha1.DiskIamBinding, opts v1.UpdateOptions) (*v1alpha1.DiskIamBinding, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(diskiambindingsResource, "status", c.ns, diskIamBinding), &v1alpha1.DiskIamBinding{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DiskIamBinding), err
}

// Delete takes name of the diskIamBinding and deletes it. Returns an error if one occurs.
func (c *FakeDiskIamBindings) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(diskiambindingsResource, c.ns, name), &v1alpha1.DiskIamBinding{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeDiskIamBindings) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(diskiambindingsResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.DiskIamBindingList{})
	return err
}

// Patch applies the patch and returns the patched diskIamBinding.
func (c *FakeDiskIamBindings) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.DiskIamBinding, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(diskiambindingsResource, c.ns, name, pt, data, subresources...), &v1alpha1.DiskIamBinding{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DiskIamBinding), err
}
