/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "kubeform.dev/provider-google-api/apis/compute/v1alpha1"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeTargetTcpProxies implements TargetTcpProxyInterface
type FakeTargetTcpProxies struct {
	Fake *FakeComputeV1alpha1
	ns   string
}

var targettcpproxiesResource = schema.GroupVersionResource{Group: "compute.google.kubeform.com", Version: "v1alpha1", Resource: "targettcpproxies"}

var targettcpproxiesKind = schema.GroupVersionKind{Group: "compute.google.kubeform.com", Version: "v1alpha1", Kind: "TargetTcpProxy"}

// Get takes name of the targetTcpProxy, and returns the corresponding targetTcpProxy object, and an error if there is any.
func (c *FakeTargetTcpProxies) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.TargetTcpProxy, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(targettcpproxiesResource, c.ns, name), &v1alpha1.TargetTcpProxy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TargetTcpProxy), err
}

// List takes label and field selectors, and returns the list of TargetTcpProxies that match those selectors.
func (c *FakeTargetTcpProxies) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.TargetTcpProxyList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(targettcpproxiesResource, targettcpproxiesKind, c.ns, opts), &v1alpha1.TargetTcpProxyList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.TargetTcpProxyList{ListMeta: obj.(*v1alpha1.TargetTcpProxyList).ListMeta}
	for _, item := range obj.(*v1alpha1.TargetTcpProxyList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested targetTcpProxies.
func (c *FakeTargetTcpProxies) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(targettcpproxiesResource, c.ns, opts))

}

// Create takes the representation of a targetTcpProxy and creates it.  Returns the server's representation of the targetTcpProxy, and an error, if there is any.
func (c *FakeTargetTcpProxies) Create(ctx context.Context, targetTcpProxy *v1alpha1.TargetTcpProxy, opts v1.CreateOptions) (result *v1alpha1.TargetTcpProxy, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(targettcpproxiesResource, c.ns, targetTcpProxy), &v1alpha1.TargetTcpProxy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TargetTcpProxy), err
}

// Update takes the representation of a targetTcpProxy and updates it. Returns the server's representation of the targetTcpProxy, and an error, if there is any.
func (c *FakeTargetTcpProxies) Update(ctx context.Context, targetTcpProxy *v1alpha1.TargetTcpProxy, opts v1.UpdateOptions) (result *v1alpha1.TargetTcpProxy, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(targettcpproxiesResource, c.ns, targetTcpProxy), &v1alpha1.TargetTcpProxy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TargetTcpProxy), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeTargetTcpProxies) UpdateStatus(ctx context.Context, targetTcpProxy *v1alpha1.TargetTcpProxy, opts v1.UpdateOptions) (*v1alpha1.TargetTcpProxy, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(targettcpproxiesResource, "status", c.ns, targetTcpProxy), &v1alpha1.TargetTcpProxy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TargetTcpProxy), err
}

// Delete takes name of the targetTcpProxy and deletes it. Returns an error if one occurs.
func (c *FakeTargetTcpProxies) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(targettcpproxiesResource, c.ns, name), &v1alpha1.TargetTcpProxy{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeTargetTcpProxies) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(targettcpproxiesResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.TargetTcpProxyList{})
	return err
}

// Patch applies the patch and returns the patched targetTcpProxy.
func (c *FakeTargetTcpProxies) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.TargetTcpProxy, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(targettcpproxiesResource, c.ns, name, pt, data, subresources...), &v1alpha1.TargetTcpProxy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TargetTcpProxy), err
}
