/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
	v1alpha1 "kubeform.dev/provider-google-api/apis/compute/v1alpha1"
)

// FakeTargetInstances implements TargetInstanceInterface
type FakeTargetInstances struct {
	Fake *FakeComputeV1alpha1
	ns   string
}

var targetinstancesResource = schema.GroupVersionResource{Group: "compute.google.kubeform.com", Version: "v1alpha1", Resource: "targetinstances"}

var targetinstancesKind = schema.GroupVersionKind{Group: "compute.google.kubeform.com", Version: "v1alpha1", Kind: "TargetInstance"}

// Get takes name of the targetInstance, and returns the corresponding targetInstance object, and an error if there is any.
func (c *FakeTargetInstances) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.TargetInstance, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(targetinstancesResource, c.ns, name), &v1alpha1.TargetInstance{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TargetInstance), err
}

// List takes label and field selectors, and returns the list of TargetInstances that match those selectors.
func (c *FakeTargetInstances) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.TargetInstanceList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(targetinstancesResource, targetinstancesKind, c.ns, opts), &v1alpha1.TargetInstanceList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.TargetInstanceList{ListMeta: obj.(*v1alpha1.TargetInstanceList).ListMeta}
	for _, item := range obj.(*v1alpha1.TargetInstanceList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested targetInstances.
func (c *FakeTargetInstances) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(targetinstancesResource, c.ns, opts))

}

// Create takes the representation of a targetInstance and creates it.  Returns the server's representation of the targetInstance, and an error, if there is any.
func (c *FakeTargetInstances) Create(ctx context.Context, targetInstance *v1alpha1.TargetInstance, opts v1.CreateOptions) (result *v1alpha1.TargetInstance, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(targetinstancesResource, c.ns, targetInstance), &v1alpha1.TargetInstance{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TargetInstance), err
}

// Update takes the representation of a targetInstance and updates it. Returns the server's representation of the targetInstance, and an error, if there is any.
func (c *FakeTargetInstances) Update(ctx context.Context, targetInstance *v1alpha1.TargetInstance, opts v1.UpdateOptions) (result *v1alpha1.TargetInstance, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(targetinstancesResource, c.ns, targetInstance), &v1alpha1.TargetInstance{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TargetInstance), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeTargetInstances) UpdateStatus(ctx context.Context, targetInstance *v1alpha1.TargetInstance, opts v1.UpdateOptions) (*v1alpha1.TargetInstance, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(targetinstancesResource, "status", c.ns, targetInstance), &v1alpha1.TargetInstance{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TargetInstance), err
}

// Delete takes name of the targetInstance and deletes it. Returns an error if one occurs.
func (c *FakeTargetInstances) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(targetinstancesResource, c.ns, name), &v1alpha1.TargetInstance{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeTargetInstances) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(targetinstancesResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.TargetInstanceList{})
	return err
}

// Patch applies the patch and returns the patched targetInstance.
func (c *FakeTargetInstances) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.TargetInstance, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(targetinstancesResource, c.ns, name, pt, data, subresources...), &v1alpha1.TargetInstance{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TargetInstance), err
}
