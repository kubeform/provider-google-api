/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
	v1alpha1 "kubeform.dev/provider-google-api/apis/compute/v1alpha1"
)

// FakeTargetSslProxies implements TargetSslProxyInterface
type FakeTargetSslProxies struct {
	Fake *FakeComputeV1alpha1
	ns   string
}

var targetsslproxiesResource = schema.GroupVersionResource{Group: "compute.google.kubeform.com", Version: "v1alpha1", Resource: "targetsslproxies"}

var targetsslproxiesKind = schema.GroupVersionKind{Group: "compute.google.kubeform.com", Version: "v1alpha1", Kind: "TargetSslProxy"}

// Get takes name of the targetSslProxy, and returns the corresponding targetSslProxy object, and an error if there is any.
func (c *FakeTargetSslProxies) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.TargetSslProxy, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(targetsslproxiesResource, c.ns, name), &v1alpha1.TargetSslProxy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TargetSslProxy), err
}

// List takes label and field selectors, and returns the list of TargetSslProxies that match those selectors.
func (c *FakeTargetSslProxies) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.TargetSslProxyList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(targetsslproxiesResource, targetsslproxiesKind, c.ns, opts), &v1alpha1.TargetSslProxyList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.TargetSslProxyList{ListMeta: obj.(*v1alpha1.TargetSslProxyList).ListMeta}
	for _, item := range obj.(*v1alpha1.TargetSslProxyList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested targetSslProxies.
func (c *FakeTargetSslProxies) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(targetsslproxiesResource, c.ns, opts))

}

// Create takes the representation of a targetSslProxy and creates it.  Returns the server's representation of the targetSslProxy, and an error, if there is any.
func (c *FakeTargetSslProxies) Create(ctx context.Context, targetSslProxy *v1alpha1.TargetSslProxy, opts v1.CreateOptions) (result *v1alpha1.TargetSslProxy, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(targetsslproxiesResource, c.ns, targetSslProxy), &v1alpha1.TargetSslProxy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TargetSslProxy), err
}

// Update takes the representation of a targetSslProxy and updates it. Returns the server's representation of the targetSslProxy, and an error, if there is any.
func (c *FakeTargetSslProxies) Update(ctx context.Context, targetSslProxy *v1alpha1.TargetSslProxy, opts v1.UpdateOptions) (result *v1alpha1.TargetSslProxy, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(targetsslproxiesResource, c.ns, targetSslProxy), &v1alpha1.TargetSslProxy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TargetSslProxy), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeTargetSslProxies) UpdateStatus(ctx context.Context, targetSslProxy *v1alpha1.TargetSslProxy, opts v1.UpdateOptions) (*v1alpha1.TargetSslProxy, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(targetsslproxiesResource, "status", c.ns, targetSslProxy), &v1alpha1.TargetSslProxy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TargetSslProxy), err
}

// Delete takes name of the targetSslProxy and deletes it. Returns an error if one occurs.
func (c *FakeTargetSslProxies) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(targetsslproxiesResource, c.ns, name), &v1alpha1.TargetSslProxy{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeTargetSslProxies) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(targetsslproxiesResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.TargetSslProxyList{})
	return err
}

// Patch applies the patch and returns the patched targetSslProxy.
func (c *FakeTargetSslProxies) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.TargetSslProxy, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(targetsslproxiesResource, c.ns, name, pt, data, subresources...), &v1alpha1.TargetSslProxy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TargetSslProxy), err
}
