/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "kubeform.dev/provider-google-api/apis/pubsub/v1alpha1"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeTopicIamPolicies implements TopicIamPolicyInterface
type FakeTopicIamPolicies struct {
	Fake *FakePubsubV1alpha1
	ns   string
}

var topiciampoliciesResource = schema.GroupVersionResource{Group: "pubsub.google.kubeform.com", Version: "v1alpha1", Resource: "topiciampolicies"}

var topiciampoliciesKind = schema.GroupVersionKind{Group: "pubsub.google.kubeform.com", Version: "v1alpha1", Kind: "TopicIamPolicy"}

// Get takes name of the topicIamPolicy, and returns the corresponding topicIamPolicy object, and an error if there is any.
func (c *FakeTopicIamPolicies) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.TopicIamPolicy, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(topiciampoliciesResource, c.ns, name), &v1alpha1.TopicIamPolicy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TopicIamPolicy), err
}

// List takes label and field selectors, and returns the list of TopicIamPolicies that match those selectors.
func (c *FakeTopicIamPolicies) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.TopicIamPolicyList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(topiciampoliciesResource, topiciampoliciesKind, c.ns, opts), &v1alpha1.TopicIamPolicyList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.TopicIamPolicyList{ListMeta: obj.(*v1alpha1.TopicIamPolicyList).ListMeta}
	for _, item := range obj.(*v1alpha1.TopicIamPolicyList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested topicIamPolicies.
func (c *FakeTopicIamPolicies) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(topiciampoliciesResource, c.ns, opts))

}

// Create takes the representation of a topicIamPolicy and creates it.  Returns the server's representation of the topicIamPolicy, and an error, if there is any.
func (c *FakeTopicIamPolicies) Create(ctx context.Context, topicIamPolicy *v1alpha1.TopicIamPolicy, opts v1.CreateOptions) (result *v1alpha1.TopicIamPolicy, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(topiciampoliciesResource, c.ns, topicIamPolicy), &v1alpha1.TopicIamPolicy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TopicIamPolicy), err
}

// Update takes the representation of a topicIamPolicy and updates it. Returns the server's representation of the topicIamPolicy, and an error, if there is any.
func (c *FakeTopicIamPolicies) Update(ctx context.Context, topicIamPolicy *v1alpha1.TopicIamPolicy, opts v1.UpdateOptions) (result *v1alpha1.TopicIamPolicy, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(topiciampoliciesResource, c.ns, topicIamPolicy), &v1alpha1.TopicIamPolicy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TopicIamPolicy), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeTopicIamPolicies) UpdateStatus(ctx context.Context, topicIamPolicy *v1alpha1.TopicIamPolicy, opts v1.UpdateOptions) (*v1alpha1.TopicIamPolicy, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(topiciampoliciesResource, "status", c.ns, topicIamPolicy), &v1alpha1.TopicIamPolicy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TopicIamPolicy), err
}

// Delete takes name of the topicIamPolicy and deletes it. Returns an error if one occurs.
func (c *FakeTopicIamPolicies) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(topiciampoliciesResource, c.ns, name), &v1alpha1.TopicIamPolicy{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeTopicIamPolicies) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(topiciampoliciesResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.TopicIamPolicyList{})
	return err
}

// Patch applies the patch and returns the patched topicIamPolicy.
func (c *FakeTopicIamPolicies) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.TopicIamPolicy, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(topiciampoliciesResource, c.ns, name, pt, data, subresources...), &v1alpha1.TopicIamPolicy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TopicIamPolicy), err
}
