/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
	v1alpha1 "kubeform.dev/provider-google-api/apis/dataproc/v1alpha1"
)

// FakeClusterIamBindings implements ClusterIamBindingInterface
type FakeClusterIamBindings struct {
	Fake *FakeDataprocV1alpha1
	ns   string
}

var clusteriambindingsResource = schema.GroupVersionResource{Group: "dataproc.google.kubeform.com", Version: "v1alpha1", Resource: "clusteriambindings"}

var clusteriambindingsKind = schema.GroupVersionKind{Group: "dataproc.google.kubeform.com", Version: "v1alpha1", Kind: "ClusterIamBinding"}

// Get takes name of the clusterIamBinding, and returns the corresponding clusterIamBinding object, and an error if there is any.
func (c *FakeClusterIamBindings) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.ClusterIamBinding, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(clusteriambindingsResource, c.ns, name), &v1alpha1.ClusterIamBinding{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.ClusterIamBinding), err
}

// List takes label and field selectors, and returns the list of ClusterIamBindings that match those selectors.
func (c *FakeClusterIamBindings) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.ClusterIamBindingList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(clusteriambindingsResource, clusteriambindingsKind, c.ns, opts), &v1alpha1.ClusterIamBindingList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.ClusterIamBindingList{ListMeta: obj.(*v1alpha1.ClusterIamBindingList).ListMeta}
	for _, item := range obj.(*v1alpha1.ClusterIamBindingList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested clusterIamBindings.
func (c *FakeClusterIamBindings) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(clusteriambindingsResource, c.ns, opts))

}

// Create takes the representation of a clusterIamBinding and creates it.  Returns the server's representation of the clusterIamBinding, and an error, if there is any.
func (c *FakeClusterIamBindings) Create(ctx context.Context, clusterIamBinding *v1alpha1.ClusterIamBinding, opts v1.CreateOptions) (result *v1alpha1.ClusterIamBinding, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(clusteriambindingsResource, c.ns, clusterIamBinding), &v1alpha1.ClusterIamBinding{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.ClusterIamBinding), err
}

// Update takes the representation of a clusterIamBinding and updates it. Returns the server's representation of the clusterIamBinding, and an error, if there is any.
func (c *FakeClusterIamBindings) Update(ctx context.Context, clusterIamBinding *v1alpha1.ClusterIamBinding, opts v1.UpdateOptions) (result *v1alpha1.ClusterIamBinding, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(clusteriambindingsResource, c.ns, clusterIamBinding), &v1alpha1.ClusterIamBinding{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.ClusterIamBinding), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeClusterIamBindings) UpdateStatus(ctx context.Context, clusterIamBinding *v1alpha1.ClusterIamBinding, opts v1.UpdateOptions) (*v1alpha1.ClusterIamBinding, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(clusteriambindingsResource, "status", c.ns, clusterIamBinding), &v1alpha1.ClusterIamBinding{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.ClusterIamBinding), err
}

// Delete takes name of the clusterIamBinding and deletes it. Returns an error if one occurs.
func (c *FakeClusterIamBindings) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(clusteriambindingsResource, c.ns, name), &v1alpha1.ClusterIamBinding{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeClusterIamBindings) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(clusteriambindingsResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.ClusterIamBindingList{})
	return err
}

// Patch applies the patch and returns the patched clusterIamBinding.
func (c *FakeClusterIamBindings) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.ClusterIamBinding, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(clusteriambindingsResource, c.ns, name, pt, data, subresources...), &v1alpha1.ClusterIamBinding{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.ClusterIamBinding), err
}
