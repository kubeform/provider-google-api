/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "kubeform.dev/provider-google-api/apis/endpoints/v1alpha1"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeServiceIamPolicies implements ServiceIamPolicyInterface
type FakeServiceIamPolicies struct {
	Fake *FakeEndpointsV1alpha1
	ns   string
}

var serviceiampoliciesResource = schema.GroupVersionResource{Group: "endpoints.google.kubeform.com", Version: "v1alpha1", Resource: "serviceiampolicies"}

var serviceiampoliciesKind = schema.GroupVersionKind{Group: "endpoints.google.kubeform.com", Version: "v1alpha1", Kind: "ServiceIamPolicy"}

// Get takes name of the serviceIamPolicy, and returns the corresponding serviceIamPolicy object, and an error if there is any.
func (c *FakeServiceIamPolicies) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.ServiceIamPolicy, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(serviceiampoliciesResource, c.ns, name), &v1alpha1.ServiceIamPolicy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.ServiceIamPolicy), err
}

// List takes label and field selectors, and returns the list of ServiceIamPolicies that match those selectors.
func (c *FakeServiceIamPolicies) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.ServiceIamPolicyList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(serviceiampoliciesResource, serviceiampoliciesKind, c.ns, opts), &v1alpha1.ServiceIamPolicyList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.ServiceIamPolicyList{ListMeta: obj.(*v1alpha1.ServiceIamPolicyList).ListMeta}
	for _, item := range obj.(*v1alpha1.ServiceIamPolicyList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested serviceIamPolicies.
func (c *FakeServiceIamPolicies) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(serviceiampoliciesResource, c.ns, opts))

}

// Create takes the representation of a serviceIamPolicy and creates it.  Returns the server's representation of the serviceIamPolicy, and an error, if there is any.
func (c *FakeServiceIamPolicies) Create(ctx context.Context, serviceIamPolicy *v1alpha1.ServiceIamPolicy, opts v1.CreateOptions) (result *v1alpha1.ServiceIamPolicy, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(serviceiampoliciesResource, c.ns, serviceIamPolicy), &v1alpha1.ServiceIamPolicy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.ServiceIamPolicy), err
}

// Update takes the representation of a serviceIamPolicy and updates it. Returns the server's representation of the serviceIamPolicy, and an error, if there is any.
func (c *FakeServiceIamPolicies) Update(ctx context.Context, serviceIamPolicy *v1alpha1.ServiceIamPolicy, opts v1.UpdateOptions) (result *v1alpha1.ServiceIamPolicy, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(serviceiampoliciesResource, c.ns, serviceIamPolicy), &v1alpha1.ServiceIamPolicy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.ServiceIamPolicy), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeServiceIamPolicies) UpdateStatus(ctx context.Context, serviceIamPolicy *v1alpha1.ServiceIamPolicy, opts v1.UpdateOptions) (*v1alpha1.ServiceIamPolicy, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(serviceiampoliciesResource, "status", c.ns, serviceIamPolicy), &v1alpha1.ServiceIamPolicy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.ServiceIamPolicy), err
}

// Delete takes name of the serviceIamPolicy and deletes it. Returns an error if one occurs.
func (c *FakeServiceIamPolicies) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(serviceiampoliciesResource, c.ns, name), &v1alpha1.ServiceIamPolicy{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeServiceIamPolicies) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(serviceiampoliciesResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.ServiceIamPolicyList{})
	return err
}

// Patch applies the patch and returns the patched serviceIamPolicy.
func (c *FakeServiceIamPolicies) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.ServiceIamPolicy, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(serviceiampoliciesResource, c.ns, name, pt, data, subresources...), &v1alpha1.ServiceIamPolicy{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.ServiceIamPolicy), err
}
