/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
	v1alpha1 "kubeform.dev/provider-google-api/apis/bigquery/v1alpha1"
)

// FakeTableIamBindings implements TableIamBindingInterface
type FakeTableIamBindings struct {
	Fake *FakeBigqueryV1alpha1
	ns   string
}

var tableiambindingsResource = schema.GroupVersionResource{Group: "bigquery.google.kubeform.com", Version: "v1alpha1", Resource: "tableiambindings"}

var tableiambindingsKind = schema.GroupVersionKind{Group: "bigquery.google.kubeform.com", Version: "v1alpha1", Kind: "TableIamBinding"}

// Get takes name of the tableIamBinding, and returns the corresponding tableIamBinding object, and an error if there is any.
func (c *FakeTableIamBindings) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.TableIamBinding, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(tableiambindingsResource, c.ns, name), &v1alpha1.TableIamBinding{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TableIamBinding), err
}

// List takes label and field selectors, and returns the list of TableIamBindings that match those selectors.
func (c *FakeTableIamBindings) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.TableIamBindingList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(tableiambindingsResource, tableiambindingsKind, c.ns, opts), &v1alpha1.TableIamBindingList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.TableIamBindingList{ListMeta: obj.(*v1alpha1.TableIamBindingList).ListMeta}
	for _, item := range obj.(*v1alpha1.TableIamBindingList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested tableIamBindings.
func (c *FakeTableIamBindings) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(tableiambindingsResource, c.ns, opts))

}

// Create takes the representation of a tableIamBinding and creates it.  Returns the server's representation of the tableIamBinding, and an error, if there is any.
func (c *FakeTableIamBindings) Create(ctx context.Context, tableIamBinding *v1alpha1.TableIamBinding, opts v1.CreateOptions) (result *v1alpha1.TableIamBinding, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(tableiambindingsResource, c.ns, tableIamBinding), &v1alpha1.TableIamBinding{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TableIamBinding), err
}

// Update takes the representation of a tableIamBinding and updates it. Returns the server's representation of the tableIamBinding, and an error, if there is any.
func (c *FakeTableIamBindings) Update(ctx context.Context, tableIamBinding *v1alpha1.TableIamBinding, opts v1.UpdateOptions) (result *v1alpha1.TableIamBinding, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(tableiambindingsResource, c.ns, tableIamBinding), &v1alpha1.TableIamBinding{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TableIamBinding), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeTableIamBindings) UpdateStatus(ctx context.Context, tableIamBinding *v1alpha1.TableIamBinding, opts v1.UpdateOptions) (*v1alpha1.TableIamBinding, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(tableiambindingsResource, "status", c.ns, tableIamBinding), &v1alpha1.TableIamBinding{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TableIamBinding), err
}

// Delete takes name of the tableIamBinding and deletes it. Returns an error if one occurs.
func (c *FakeTableIamBindings) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(tableiambindingsResource, c.ns, name), &v1alpha1.TableIamBinding{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeTableIamBindings) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(tableiambindingsResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.TableIamBindingList{})
	return err
}

// Patch applies the patch and returns the patched tableIamBinding.
func (c *FakeTableIamBindings) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.TableIamBinding, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(tableiambindingsResource, c.ns, name, pt, data, subresources...), &v1alpha1.TableIamBinding{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TableIamBinding), err
}
