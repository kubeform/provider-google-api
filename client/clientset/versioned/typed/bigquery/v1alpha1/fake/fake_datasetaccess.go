/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
	v1alpha1 "kubeform.dev/provider-google-api/apis/bigquery/v1alpha1"
)

// FakeDatasetAccesses implements DatasetAccessInterface
type FakeDatasetAccesses struct {
	Fake *FakeBigqueryV1alpha1
	ns   string
}

var datasetaccessesResource = schema.GroupVersionResource{Group: "bigquery.google.kubeform.com", Version: "v1alpha1", Resource: "datasetaccesses"}

var datasetaccessesKind = schema.GroupVersionKind{Group: "bigquery.google.kubeform.com", Version: "v1alpha1", Kind: "DatasetAccess"}

// Get takes name of the datasetAccess, and returns the corresponding datasetAccess object, and an error if there is any.
func (c *FakeDatasetAccesses) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.DatasetAccess, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(datasetaccessesResource, c.ns, name), &v1alpha1.DatasetAccess{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DatasetAccess), err
}

// List takes label and field selectors, and returns the list of DatasetAccesses that match those selectors.
func (c *FakeDatasetAccesses) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.DatasetAccessList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(datasetaccessesResource, datasetaccessesKind, c.ns, opts), &v1alpha1.DatasetAccessList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.DatasetAccessList{ListMeta: obj.(*v1alpha1.DatasetAccessList).ListMeta}
	for _, item := range obj.(*v1alpha1.DatasetAccessList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested datasetAccesses.
func (c *FakeDatasetAccesses) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(datasetaccessesResource, c.ns, opts))

}

// Create takes the representation of a datasetAccess and creates it.  Returns the server's representation of the datasetAccess, and an error, if there is any.
func (c *FakeDatasetAccesses) Create(ctx context.Context, datasetAccess *v1alpha1.DatasetAccess, opts v1.CreateOptions) (result *v1alpha1.DatasetAccess, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(datasetaccessesResource, c.ns, datasetAccess), &v1alpha1.DatasetAccess{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DatasetAccess), err
}

// Update takes the representation of a datasetAccess and updates it. Returns the server's representation of the datasetAccess, and an error, if there is any.
func (c *FakeDatasetAccesses) Update(ctx context.Context, datasetAccess *v1alpha1.DatasetAccess, opts v1.UpdateOptions) (result *v1alpha1.DatasetAccess, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(datasetaccessesResource, c.ns, datasetAccess), &v1alpha1.DatasetAccess{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DatasetAccess), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeDatasetAccesses) UpdateStatus(ctx context.Context, datasetAccess *v1alpha1.DatasetAccess, opts v1.UpdateOptions) (*v1alpha1.DatasetAccess, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(datasetaccessesResource, "status", c.ns, datasetAccess), &v1alpha1.DatasetAccess{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DatasetAccess), err
}

// Delete takes name of the datasetAccess and deletes it. Returns an error if one occurs.
func (c *FakeDatasetAccesses) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(datasetaccessesResource, c.ns, name), &v1alpha1.DatasetAccess{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeDatasetAccesses) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(datasetaccessesResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.DatasetAccessList{})
	return err
}

// Patch applies the patch and returns the patched datasetAccess.
func (c *FakeDatasetAccesses) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.DatasetAccess, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(datasetaccessesResource, c.ns, name, pt, data, subresources...), &v1alpha1.DatasetAccess{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DatasetAccess), err
}
