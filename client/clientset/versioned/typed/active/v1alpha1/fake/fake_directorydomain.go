/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "kubeform.dev/provider-google-api/apis/active/v1alpha1"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeDirectoryDomains implements DirectoryDomainInterface
type FakeDirectoryDomains struct {
	Fake *FakeActiveV1alpha1
	ns   string
}

var directorydomainsResource = schema.GroupVersionResource{Group: "active.google.kubeform.com", Version: "v1alpha1", Resource: "directorydomains"}

var directorydomainsKind = schema.GroupVersionKind{Group: "active.google.kubeform.com", Version: "v1alpha1", Kind: "DirectoryDomain"}

// Get takes name of the directoryDomain, and returns the corresponding directoryDomain object, and an error if there is any.
func (c *FakeDirectoryDomains) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.DirectoryDomain, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(directorydomainsResource, c.ns, name), &v1alpha1.DirectoryDomain{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DirectoryDomain), err
}

// List takes label and field selectors, and returns the list of DirectoryDomains that match those selectors.
func (c *FakeDirectoryDomains) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.DirectoryDomainList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(directorydomainsResource, directorydomainsKind, c.ns, opts), &v1alpha1.DirectoryDomainList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.DirectoryDomainList{ListMeta: obj.(*v1alpha1.DirectoryDomainList).ListMeta}
	for _, item := range obj.(*v1alpha1.DirectoryDomainList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested directoryDomains.
func (c *FakeDirectoryDomains) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(directorydomainsResource, c.ns, opts))

}

// Create takes the representation of a directoryDomain and creates it.  Returns the server's representation of the directoryDomain, and an error, if there is any.
func (c *FakeDirectoryDomains) Create(ctx context.Context, directoryDomain *v1alpha1.DirectoryDomain, opts v1.CreateOptions) (result *v1alpha1.DirectoryDomain, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(directorydomainsResource, c.ns, directoryDomain), &v1alpha1.DirectoryDomain{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DirectoryDomain), err
}

// Update takes the representation of a directoryDomain and updates it. Returns the server's representation of the directoryDomain, and an error, if there is any.
func (c *FakeDirectoryDomains) Update(ctx context.Context, directoryDomain *v1alpha1.DirectoryDomain, opts v1.UpdateOptions) (result *v1alpha1.DirectoryDomain, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(directorydomainsResource, c.ns, directoryDomain), &v1alpha1.DirectoryDomain{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DirectoryDomain), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeDirectoryDomains) UpdateStatus(ctx context.Context, directoryDomain *v1alpha1.DirectoryDomain, opts v1.UpdateOptions) (*v1alpha1.DirectoryDomain, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(directorydomainsResource, "status", c.ns, directoryDomain), &v1alpha1.DirectoryDomain{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DirectoryDomain), err
}

// Delete takes name of the directoryDomain and deletes it. Returns an error if one occurs.
func (c *FakeDirectoryDomains) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(directorydomainsResource, c.ns, name), &v1alpha1.DirectoryDomain{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeDirectoryDomains) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(directorydomainsResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.DirectoryDomainList{})
	return err
}

// Patch applies the patch and returns the patched directoryDomain.
func (c *FakeDirectoryDomains) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.DirectoryDomain, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(directorydomainsResource, c.ns, name, pt, data, subresources...), &v1alpha1.DirectoryDomain{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.DirectoryDomain), err
}
