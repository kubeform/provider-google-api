/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
	v1alpha1 "kubeform.dev/provider-google-api/apis/logging/v1alpha1"
)

// FakeProjectExclusions implements ProjectExclusionInterface
type FakeProjectExclusions struct {
	Fake *FakeLoggingV1alpha1
	ns   string
}

var projectexclusionsResource = schema.GroupVersionResource{Group: "logging.google.kubeform.com", Version: "v1alpha1", Resource: "projectexclusions"}

var projectexclusionsKind = schema.GroupVersionKind{Group: "logging.google.kubeform.com", Version: "v1alpha1", Kind: "ProjectExclusion"}

// Get takes name of the projectExclusion, and returns the corresponding projectExclusion object, and an error if there is any.
func (c *FakeProjectExclusions) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.ProjectExclusion, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(projectexclusionsResource, c.ns, name), &v1alpha1.ProjectExclusion{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.ProjectExclusion), err
}

// List takes label and field selectors, and returns the list of ProjectExclusions that match those selectors.
func (c *FakeProjectExclusions) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.ProjectExclusionList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(projectexclusionsResource, projectexclusionsKind, c.ns, opts), &v1alpha1.ProjectExclusionList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.ProjectExclusionList{ListMeta: obj.(*v1alpha1.ProjectExclusionList).ListMeta}
	for _, item := range obj.(*v1alpha1.ProjectExclusionList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested projectExclusions.
func (c *FakeProjectExclusions) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(projectexclusionsResource, c.ns, opts))

}

// Create takes the representation of a projectExclusion and creates it.  Returns the server's representation of the projectExclusion, and an error, if there is any.
func (c *FakeProjectExclusions) Create(ctx context.Context, projectExclusion *v1alpha1.ProjectExclusion, opts v1.CreateOptions) (result *v1alpha1.ProjectExclusion, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(projectexclusionsResource, c.ns, projectExclusion), &v1alpha1.ProjectExclusion{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.ProjectExclusion), err
}

// Update takes the representation of a projectExclusion and updates it. Returns the server's representation of the projectExclusion, and an error, if there is any.
func (c *FakeProjectExclusions) Update(ctx context.Context, projectExclusion *v1alpha1.ProjectExclusion, opts v1.UpdateOptions) (result *v1alpha1.ProjectExclusion, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(projectexclusionsResource, c.ns, projectExclusion), &v1alpha1.ProjectExclusion{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.ProjectExclusion), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeProjectExclusions) UpdateStatus(ctx context.Context, projectExclusion *v1alpha1.ProjectExclusion, opts v1.UpdateOptions) (*v1alpha1.ProjectExclusion, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(projectexclusionsResource, "status", c.ns, projectExclusion), &v1alpha1.ProjectExclusion{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.ProjectExclusion), err
}

// Delete takes name of the projectExclusion and deletes it. Returns an error if one occurs.
func (c *FakeProjectExclusions) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(projectexclusionsResource, c.ns, name), &v1alpha1.ProjectExclusion{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeProjectExclusions) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(projectexclusionsResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.ProjectExclusionList{})
	return err
}

// Patch applies the patch and returns the patched projectExclusion.
func (c *FakeProjectExclusions) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.ProjectExclusion, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(projectexclusionsResource, c.ns, name, pt, data, subresources...), &v1alpha1.ProjectExclusion{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.ProjectExclusion), err
}
