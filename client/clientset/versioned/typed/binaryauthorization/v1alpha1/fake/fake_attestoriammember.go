/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "kubeform.dev/provider-google-api/apis/binaryauthorization/v1alpha1"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeAttestorIamMembers implements AttestorIamMemberInterface
type FakeAttestorIamMembers struct {
	Fake *FakeBinaryauthorizationV1alpha1
	ns   string
}

var attestoriammembersResource = schema.GroupVersionResource{Group: "binaryauthorization.google.kubeform.com", Version: "v1alpha1", Resource: "attestoriammembers"}

var attestoriammembersKind = schema.GroupVersionKind{Group: "binaryauthorization.google.kubeform.com", Version: "v1alpha1", Kind: "AttestorIamMember"}

// Get takes name of the attestorIamMember, and returns the corresponding attestorIamMember object, and an error if there is any.
func (c *FakeAttestorIamMembers) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.AttestorIamMember, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(attestoriammembersResource, c.ns, name), &v1alpha1.AttestorIamMember{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.AttestorIamMember), err
}

// List takes label and field selectors, and returns the list of AttestorIamMembers that match those selectors.
func (c *FakeAttestorIamMembers) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.AttestorIamMemberList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(attestoriammembersResource, attestoriammembersKind, c.ns, opts), &v1alpha1.AttestorIamMemberList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.AttestorIamMemberList{ListMeta: obj.(*v1alpha1.AttestorIamMemberList).ListMeta}
	for _, item := range obj.(*v1alpha1.AttestorIamMemberList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested attestorIamMembers.
func (c *FakeAttestorIamMembers) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(attestoriammembersResource, c.ns, opts))

}

// Create takes the representation of a attestorIamMember and creates it.  Returns the server's representation of the attestorIamMember, and an error, if there is any.
func (c *FakeAttestorIamMembers) Create(ctx context.Context, attestorIamMember *v1alpha1.AttestorIamMember, opts v1.CreateOptions) (result *v1alpha1.AttestorIamMember, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(attestoriammembersResource, c.ns, attestorIamMember), &v1alpha1.AttestorIamMember{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.AttestorIamMember), err
}

// Update takes the representation of a attestorIamMember and updates it. Returns the server's representation of the attestorIamMember, and an error, if there is any.
func (c *FakeAttestorIamMembers) Update(ctx context.Context, attestorIamMember *v1alpha1.AttestorIamMember, opts v1.UpdateOptions) (result *v1alpha1.AttestorIamMember, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(attestoriammembersResource, c.ns, attestorIamMember), &v1alpha1.AttestorIamMember{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.AttestorIamMember), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeAttestorIamMembers) UpdateStatus(ctx context.Context, attestorIamMember *v1alpha1.AttestorIamMember, opts v1.UpdateOptions) (*v1alpha1.AttestorIamMember, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(attestoriammembersResource, "status", c.ns, attestorIamMember), &v1alpha1.AttestorIamMember{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.AttestorIamMember), err
}

// Delete takes name of the attestorIamMember and deletes it. Returns an error if one occurs.
func (c *FakeAttestorIamMembers) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(attestoriammembersResource, c.ns, name), &v1alpha1.AttestorIamMember{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeAttestorIamMembers) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(attestoriammembersResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.AttestorIamMemberList{})
	return err
}

// Patch applies the patch and returns the patched attestorIamMember.
func (c *FakeAttestorIamMembers) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.AttestorIamMember, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(attestoriammembersResource, c.ns, name, pt, data, subresources...), &v1alpha1.AttestorIamMember{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.AttestorIamMember), err
}
