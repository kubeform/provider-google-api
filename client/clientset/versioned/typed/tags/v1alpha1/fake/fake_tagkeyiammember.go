/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
	v1alpha1 "kubeform.dev/provider-google-api/apis/tags/v1alpha1"
)

// FakeTagKeyIamMembers implements TagKeyIamMemberInterface
type FakeTagKeyIamMembers struct {
	Fake *FakeTagsV1alpha1
	ns   string
}

var tagkeyiammembersResource = schema.GroupVersionResource{Group: "tags.google.kubeform.com", Version: "v1alpha1", Resource: "tagkeyiammembers"}

var tagkeyiammembersKind = schema.GroupVersionKind{Group: "tags.google.kubeform.com", Version: "v1alpha1", Kind: "TagKeyIamMember"}

// Get takes name of the tagKeyIamMember, and returns the corresponding tagKeyIamMember object, and an error if there is any.
func (c *FakeTagKeyIamMembers) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.TagKeyIamMember, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(tagkeyiammembersResource, c.ns, name), &v1alpha1.TagKeyIamMember{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TagKeyIamMember), err
}

// List takes label and field selectors, and returns the list of TagKeyIamMembers that match those selectors.
func (c *FakeTagKeyIamMembers) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.TagKeyIamMemberList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(tagkeyiammembersResource, tagkeyiammembersKind, c.ns, opts), &v1alpha1.TagKeyIamMemberList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.TagKeyIamMemberList{ListMeta: obj.(*v1alpha1.TagKeyIamMemberList).ListMeta}
	for _, item := range obj.(*v1alpha1.TagKeyIamMemberList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested tagKeyIamMembers.
func (c *FakeTagKeyIamMembers) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(tagkeyiammembersResource, c.ns, opts))

}

// Create takes the representation of a tagKeyIamMember and creates it.  Returns the server's representation of the tagKeyIamMember, and an error, if there is any.
func (c *FakeTagKeyIamMembers) Create(ctx context.Context, tagKeyIamMember *v1alpha1.TagKeyIamMember, opts v1.CreateOptions) (result *v1alpha1.TagKeyIamMember, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(tagkeyiammembersResource, c.ns, tagKeyIamMember), &v1alpha1.TagKeyIamMember{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TagKeyIamMember), err
}

// Update takes the representation of a tagKeyIamMember and updates it. Returns the server's representation of the tagKeyIamMember, and an error, if there is any.
func (c *FakeTagKeyIamMembers) Update(ctx context.Context, tagKeyIamMember *v1alpha1.TagKeyIamMember, opts v1.UpdateOptions) (result *v1alpha1.TagKeyIamMember, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(tagkeyiammembersResource, c.ns, tagKeyIamMember), &v1alpha1.TagKeyIamMember{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TagKeyIamMember), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeTagKeyIamMembers) UpdateStatus(ctx context.Context, tagKeyIamMember *v1alpha1.TagKeyIamMember, opts v1.UpdateOptions) (*v1alpha1.TagKeyIamMember, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(tagkeyiammembersResource, "status", c.ns, tagKeyIamMember), &v1alpha1.TagKeyIamMember{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TagKeyIamMember), err
}

// Delete takes name of the tagKeyIamMember and deletes it. Returns an error if one occurs.
func (c *FakeTagKeyIamMembers) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(tagkeyiammembersResource, c.ns, name), &v1alpha1.TagKeyIamMember{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeTagKeyIamMembers) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(tagkeyiammembersResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.TagKeyIamMemberList{})
	return err
}

// Patch applies the patch and returns the patched tagKeyIamMember.
func (c *FakeTagKeyIamMembers) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.TagKeyIamMember, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(tagkeyiammembersResource, c.ns, name, pt, data, subresources...), &v1alpha1.TagKeyIamMember{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.TagKeyIamMember), err
}
